        ...
        public static async void DoIt()
        {
            ...
            do
            {
                ...
                if (!string.IsNullOrEmpty(input))
                {
                    ...
                    //HttpClient httpClient, string downloadPath, string destinationPath, DownloadProgressHandler progress, Func<HttpRequestMessage> requestMessageBuilder = null
                    Console.WriteLine($"{destination}");
                   await DownloadWithProgress.ExecuteAsync(new HttpClient(), input, destination, (total, downloaded, progress) =>
                 {
                     Console.WriteLine($"Downloading...{progress/total*100}% complete ({downloaded: NO} bytes)");
                 });
                }
